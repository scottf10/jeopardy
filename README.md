...
Apply every file in `supabase/migrations/` in filename order. The first five migrations are the shared teacher/auth foundation; `0006_jeopardy_schema.sql` adds the isolated Jeopardy tables and RPCs, `0008_buzzer_timer.sql` adds the timed buzzer workflow, `0010_final_submission.sql` adds the atomic Final Jeopardy submission flow, and `0011_pause_buzzer.sql` adds synchronized timer pausing.
