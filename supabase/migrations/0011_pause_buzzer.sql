alter table public.jeopardy_sessions
  add column buzz_paused_remaining_ms integer,
  add constraint jeopardy_sessions_paused_remaining check (
    buzz_paused_remaining_ms is null or buzz_paused_remaining_ms > 0
  );

create or replace function public.jeopardy_set_buzz_paused(p_session_id uuid, p_paused boolean)
returns void
language plpgsql
security definer
...
begin
  ...
  if selected.state <> 'clue' or selected.buzz_team_id is null or selected.buzz_started_at is null then
    raise exception 'No active buzzer timer';
  end if;

  if p_paused then
    if selected.buzz_paused_remaining_ms is not null then return; end if;
    remaining_ms := greatest(0, floor(extract(epoch from (
      selected.buzz_started_at + make_interval(secs => selected.buzz_duration_seconds) - changed_at
    )) * 1000)::integer);
    if remaining_ms <= 0 then raise exception 'The buzzer timer has already expired'; end if;
    update public.jeopardy_sessions
    set buzz_paused_remaining_ms = remaining_ms,
        updated_at = changed_at
    where id = selected.id;
  else
    if selected.buzz_paused_remaining_ms is null then return; end if;
    elapsed_ms := (selected.buzz_duration_seconds * 1000) - selected.buzz_paused_remaining_ms;
    update public.jeopardy_sessions
    set buzz_started_at = changed_at - (elapsed_ms * interval '1 millisecond'),
        buzz_paused_remaining_ms = null,
        updated_at = changed_at
    where id = selected.id;
  end if;
end;
...
begin
  ...
  if selected.buzz_team_id is not null then
    if selected.buzz_paused_remaining_ms is not null
       or selected.buzz_started_at + make_interval(secs => selected.buzz_duration_seconds) > buzz_time then
...
begin
  ...
  active_buzz := selected.buzz_team_id is not null
    ...
    and (selected.buzz_paused_remaining_ms is not null
      or selected.buzz_started_at + make_interval(secs => selected.buzz_duration_seconds) > checked_at);
  ...
  if active_buzz then
    ...
    remaining_ms := coalesce(selected.buzz_paused_remaining_ms, greatest(0, floor(extract(epoch from (
  ...
  return jsonb_build_object(
    ...
    'buzzer', jsonb_build_object(
      ...
      'paused', active_buzz and selected.buzz_paused_remaining_ms is not null,
...
revoke all on function public.jeopardy_set_buzz_paused(uuid, boolean) from public;
grant execute on function public.jeopardy_set_buzz_paused(uuid, boolean) to authenticated;
